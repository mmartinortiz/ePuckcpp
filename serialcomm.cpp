...
int SerialComm::writeData(const char *buf, int num_bytes, int usleep_time) {
//    int SerialComm::writeData(char *buf, int num_bytes, int usleep_time) {
    ...
    errno=0;
    int timeout = usleep_time/70;   //wait 70 useconds between every read;
                                    //the loop lasts for usleep_time/70 times
    while(bytes_written < num_bytes)
    {
        ...
        {   //if write ok
            ...
            usleep(70); // (115200/8) = 70 useconds per byte
        }
